...
```text
cargo run --locked --release -p rf-tines-lab -- memory-contact-check --output renders/contact-rk4.json
cargo run --locked --release -p rf-tines-lab -- memory-free-check --output renders/contact-rk4-free-control.json
```
