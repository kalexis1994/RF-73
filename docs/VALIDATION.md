...
## Controlled full-state mechanical loss recovery (2026-09-06)

The [mechanical loss study](MECHANICAL-LOSS.md) now recovers separate structural
and damper viscous scales from the existing nine-coordinate solver. Excitation,
damper state and full mechanical coordinates are known. The six fixed cases use
48/96 kHz refinement and three independent scale pairs. Energy is reconstructed
from endpoint states; power uses separate trapezoidal quadrature instead of the
solver's heat accumulator. Fit and held-out intervals were fixed before running.

All six cases qualify. Maximum structural/damper relative errors are
`9.187e-7` / `2.231e-6`; maximum held-out energy relative RMSE is `1.072e-6`.
Omitting damper power leaves 0.633..0.743 relative error. The retained
`references/mechanical-loss-validation.json` is 18476 bytes, SHA-256
`b7cbdb5e5070b59830baefa87248861460b8e52af2cfe117efe44f472d3e3ea1`.
No source recording, physical parameter or audible preset was fitted.

All 286 workspace release tests pass, including analysis, DSP, plugin and UI.
Four new tests cover immutable operator inspection/spatial damper power,
independent inverse rows and held-out energy, rank/contact/invalid-input
rejection, and the complete CLI study with output protection. The first operator
test required exact floating-point symmetry and encountered a `2e-18` rounding
difference; it now uses a scaled `1e-14` tolerance. No operator was changed.
Strict workspace Clippy/all targets and formatting pass; its test-loop style
fix is followed by a focused operator-test recheck.

The sibling RackForge project advanced to 0.1.16. The initial locked build
correctly refused the stale local entries; offline resolution updated only
`rackforge-plugin-sdk` and `rackforge-program-api` from 0.1.15 to 0.1.16 in
Cargo.lock. No registry package changed. Full workspace checks cover source
compatibility, not a packaged or running host test.

The next observation study should remove full-state information while retaining
known excitation/damper history, exposing modal and pickup identifiability limits.
Two global multipliers under correct known operator shapes do not establish
real-instrument material coefficients. Matrix accessors return copies for
research only; integration and physical equations are unchanged. No WAV assets
were generated by the experiment. Debug cache is cleaned after the checks;
source/listening files and the existing packaged plugin are preserved.

## Reduced mechanical observation study (2026-09-06)

After commit `633b3db`, the [reduced-observation study](REDUCED-MECHANICAL-LOSS.md)
compares full state, lowest 1/3/6/9 structural modes and an instantaneous pickup
lift on the same six trajectories. Only the laboratory receives a read-only
tick callback; physical equations, profiles and the existing simulator sequence
are unchanged. Identity rows match the previous mechanical receipt exactly.

The 101844-byte `references/reduced-mechanical-loss-validation.json` retains all
36 observation results, SHA-256
`4f500468ed28d824662342a833c908286e42ed3a755626dbcc8e39999b28b994`.
All full-state/nine-mode controls pass. Three-mode and six-mode observations
recover both prescribed scales and pass held-out consistency in all six cases;
one-mode and instantaneous pickup reconstructions fail all six. Maximum
three-mode structural/damper relative errors are 0.003279 / 0.000144. Six-mode
maximum damper relative error is 2.091e-5. Reduced-state bias need not diminish
with time-step refinement. No count, row or threshold was changed after the run.

All 106 laboratory release tests pass (77 unit and 29 CLI integration tests),
including the original mechanical-loss command. Two new unit tests cover
mass-orthogonal/idempotent and sign-invariant projections, exact nine-mode
reconstruction, pickup-port preservation and an instantaneous invisible motion.
The new CLI regression retains all observations and shared-trajectory controls,
checks separate fit/recovery status and protects outputs. Strict laboratory
Clippy/all targets, formatting and whitespace checks pass. No additional full
workspace, host or listening test is claimed for this laboratory-only change.

Exact simulator modal coordinates are oracle information; this is not audio
mode extraction, a dynamic pickup observer or a reduced real-time engine. Next
test reconstruction from pickup time history with noise/omitted-mode controls.
