...
Open RF-Tines Research in RackForge PLAY. The panel has **Reference A** and **Candidate B** selectors, a **Profile** selector, **Listen to A/B** buttons and shared **Output gain**. Its connection indicator confirms receipt of valid live parameters; it does not indicate audio-device health. RackForge PRESETS saves and restores comparisons. The stable factory program ID remains `research-direct` (Pickup Lab).

On LITTLE controllers, open CONTROLS / **Pickup Comparison**. **Pickup A** and **Pickup B** select the variants; **Listen to B** chooses B when on, A when off; **Profile** selects the mechanics shared by both. The **Output** page controls gain. Both surfaces use the same host parameters and MIDI Links. The screen panel synchronizes at 500 ms intervals and after acknowledged writes. It serializes writes, coalesces rapid edits, rejects stale responses and disables editing after a five-second timeout until a valid snapshot arrives. No MIDI notes are generated by the panel.
...
The **Profile** parameter (index 4) chooses the mechanics under both pickups:

| Profile | First / bar / third partial T60 at A3 | Second partial strike weight | Source |
| --- | --- | --- | --- |
| Original | 5 s / 160 ms / 55 ms | −0.3 | Assumed 0.1.1 constants |
| Calibrated Sustain | 20 s / 2.3 s / 55 ms | −0.3 | [Calibrated sustain](PLAYABLE-SUSTAIN.md) |
| Calibrated | 20 s / 2.3 s / 55 ms | −0.02 | [Bar partial](PLAYABLE-BAR-PARTIAL.md) |

Switching the profile keeps every ringing note: modal and hammer states stay, and only the frequencies, losses, strike weights and contact law change from the next sample, so a held note can be compared across profiles without a restrike. The pickup geometry stays at the frozen level-matched values in every profile. The default is Original, so nothing changes until the selector is moved.

The initial comparison is Current (A) against Close Point Pole (B), listening to A, with the Original profile. Close Aperture is the fourth choice, added by the [aperture path block](PICKUP-APERTURE-PATH.md); its factor comes from the same protocol rerun with four tracks, which reproduced the first three factors exactly. Use identical MIDI gestures or a repeated MIDI phrase for controlled comparisons. Sustained-note switching is useful for body tone; replay the phrase to compare attacks. Listen for attack hardness, bell/bark balance, decay and continuity between velocities and registers. A preferred sound does not establish measured physical calibration.

The controller PROGRAMS create/edit workflow opens the declarative **RF-Tines Pickup Lab** editor, with the same five fields and live preview. Program documents saved before the profile field existed load with the Original profile. RackForge owns draft audition, save/cancel and files. Opening this editor starts a host audition lease and may reset existing notes; subsequent field previews preserve ringing motion. The screen PLAY panel uses direct parameter controls and host PRESETS; it does not duplicate the controller custom-program workflow.
...
Binary state schema 3 contains `RFRH`, little-endian version 3, an f64 gain, A and B indices, a boolean side and the profile index (20 bytes total). The processor also decodes schema 2, whose last byte was reserved at zero, as the Original profile, and the old 16-byte schema 1 with its gain and Current selected. **The pinned RackForge host rejects preset references from a different plugin version/schema before passing their bytes to the processor.** This decoder compatibility does not make old `.rfpreset` files automatically loadable in the host. Old files are retained; no host-side migration is performed here.
...
cargo clippy --locked -p rf-tines-ui --target wasm32-unknown-unknown -- -D warnings
cargo run --locked --release -p rf-tines-lab -- build-ui
cargo run --locked --release -p rf-tines-lab -- stress --laboratory
cargo run --locked --release -p rf-tines-lab -- stress --laboratory --sample-rate 192000
