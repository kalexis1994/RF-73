...
Open RF-73 Research in RackForge PLAY. The panel has **Reference A** and **Candidate B** selectors, a **Profile** selector, **Listen to A/B** buttons and shared **Output gain**. Its connection indicator confirms receipt of valid live parameters; it does not indicate audio-device health. RackForge PRESETS saves and restores comparisons. The stable factory program ID remains `research-direct` (Pickup Lab).
...
cargo clippy --locked -p rf-73-ui --target wasm32-unknown-unknown -- -D warnings
cargo run --locked --release -p rf-73-lab -- build-ui
cargo run --locked --release -p rf-73-lab -- stress --laboratory
cargo run --locked --release -p rf-73-lab -- stress --laboratory --sample-rate 192000
