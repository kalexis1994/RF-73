...
```text
cargo run --locked --release -p rf-tines-lab -- memory-modal-tail-timing --output renders/tail-timing.json
cargo run --locked --release -p rf-tines-lab -- memory-modal-stiffness-timing --output renders/stiffness-timing.json
```
