...
cargo run --locked --release -p rf-73-lab -- analyze references/audio/a3.wav --output renders/reference-analysis.json --channel 0 --note 57 --sustain-end 3
cargo run --locked --release -p rf-73-lab -- analyze references/audio/a3.wav --output renders/reference-long.json --channel 0 --note 57 --sustain-end 5 --partial-window-ms 1024
cargo run --locked --release -p rf-73-lab -- compare references/audio/a3.wav renders/a3.wav --reference-channel 0 --output renders/comparison.json
cargo run --locked --release -p rf-73-lab -- compare-partials references/audio/a3.wav renders/a3.wav --reference-channel 0 --output renders/partial-comparison.json --seconds 2 --reference-start 0.2 --candidate-start 0.2
```
...
```text
cargo run --locked --release -p rf-73-lab -- render-pickup-pair --output renders/g3-pair.wav --note 55 --velocity 0.9 --sample-rate 44100 --seconds 3 --hold 2.8
```
...
```text
cargo run --locked --release -p rf-73-lab -- sweep-pickup references/audio/a3.wav --output renders/pickup-sweep.json --note 57 --velocity 0.7 --seconds 1 --reference-start 0.1 --model-start 0.1 --gaps-mm 1,1.5,2 --offsets-mm 0.25,0.5,0.75
```
...
```text
cargo run --locked --release -p rf-73-lab -- converge --output renders/treble-convergence.json --note 100 --velocity 0.2
```
...
For the complete build/install/launch cycle, use `cargo run --locked --release -p rf-73-lab -- audition`. See [Desktop audition](AUDITION.md) for the dedicated library, settings retention and repeat-run behavior. The standalone `package` command below remains useful for producing a versioned release archive without launching a host.
...
Build RackForge's current Rust `rackforge-store` and `rackforge-core` tools in its own repository (`cargo build --locked --release -p rackforge-store -p rackforge-core`). Then from RF-73 run:

```text
cargo run --release -p rf-73-lab -- package
```
...
```text
crates/rf-73-dsp/       model, contact solver, voices, pickup and decimation
crates/rf-73-analysis/  offline audio decoding, spectra, envelopes and comparison
crates/rf-73-plugin/    SDK adapter, MIDI validation and versioned state
tools/rf-73-lab/       Rust rendering, traces, reports and stress measurements
