...
```text
cargo run --locked --release -p rf-tines-lab -- memory-modal-economical-check --output renders/contact-scheduling.json
cargo run --locked --release -p rf-tines-lab -- memory-modal-economical-timing --output renders/contact-scheduling-timing.json
```
