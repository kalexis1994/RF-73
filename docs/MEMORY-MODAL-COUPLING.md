...
```text
cargo run --locked --release -p rf-tines-lab -- memory-modal-check --output renders/memory-modal.json
cargo run --locked --release -p rf-tines-lab -- memory-modal-check --output renders/memory-modal-coarse.json --coarse
```
