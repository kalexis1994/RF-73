...
| Bank | Published content | Assessment for RF-Rhodes |
| --- | --- | --- |
...
These are playback mapping ranges, not measured strike speeds or RF-Rhodes velocities. They must not be inserted into `fit-pickup-set` as calibrated physical inputs.
...
```text
cargo run --locked --release -p rf-rhodes-lab -- analyze references/audio/jrhodes-g3-a886e6c/A_055__G3_1.wav --output renders/jrhodes-g3-layer1.json --note 55 --partial-window-ms 128
```
