...
```text
cargo run --locked --release -p rf-tines-lab -- memory-modal-free-check --output renders/memory-modal-free.json
cargo run --locked --release -p rf-tines-lab -- memory-modal-free-timing --output renders/memory-modal-free-timing.json
```
