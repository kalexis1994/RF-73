...
Research date: 2026-09-04. Baseline: RF-Tines 0.1.1, revision `f74e102`.
...
| Project | What was checked | Use for RF-Tines |
| --- | --- | --- |
...
Extend `rf-tines-analysis` with local peak detection, cross-frame association, noise-floor qualification and per-track decay fits. Preserve harmonic summaries as separate outputs. Report frequency, amplitude, time span, fit interval and uncertainty/rejection reasons. A spectral peak alone is not proof of a mechanical mode: pickup mixing can create additional components.
...
No paper or project examined establishes perceptual equivalence for RF-Tines. The immediate deliverable is a better measurement path followed by a controlled physical change.
