...
```text
cargo run --locked --release -p rf-tines-lab -- memory-modal-reimpact-check --output renders/modal-reimpact.json
```
...
Follow-up: the [shared physical checkpoint experiment](MEMORY-MODAL-CHECKPOINT.md)
passes 48 local continuations starting from identical preimpact states. This
does not repair the full-repetition failure. It motivates moving the checkpoint
back to the late impulse to isolate the preceding free/material evolution.
