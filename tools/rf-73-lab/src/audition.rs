...
// Stable host identity predates the RF-73 product name. Saved sessions and
// custom programs must continue to resolve the same instrument.
const ID: &str = "org.rackforge.rhodes";
const INSTANCE: &str = "desktop.org.rackforge.rhodes";
const MARKER: &str = "RF-73 audition library v1\n";
const LEGACY_MARKER: &str = "RF-Rhodes audition library v1\n";
...
pub fn run(args: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    let archive = run_dir.join(format!("RF-73-{}.rfplugin", env!("CARGO_PKG_VERSION")));
    build_to(&archive)?;
    ...
    verify_install(
        &installed,
        &root.join("target/wasm32-unknown-unknown/release/rf_73_plugin.wasm"),
    )?;
    ...
    if let Some(desktop) = desktop {
        ...
        println!(
            "RackForge started: PID {}. RF-73 {} installed and selected.",
            child.id(),
            env!("CARGO_PKG_VERSION")
        );
        ...
        write_json(&run_dir.join("audition.json"), &receipt)?;
        println!(
            "Prepared RF-73 {} without launching Desktop.",
            env!("CARGO_PKG_VERSION")
        );
    }
...
fn find_desktop(host: &Path) -> Result<PathBuf, Box<dyn Error>> {
    if let Some(path) =
        std::env::var_os("RF_73_DESKTOP").or_else(|| std::env::var_os("RF_RHODES_DESKTOP"))
    {
        let path = PathBuf::from(path);
        if !path.is_file() {
            return Err("RF_73_DESKTOP must name an existing RackForge executable".into());
        }
    ...
    .ok_or("build RackForge Desktop first, or set RF_73_DESKTOP to its executable")?;
...
fn own_library(library: &Path) -> Result<(), Box<dyn Error>> {
    if library.exists() {
        ...
        let marker = library.join(".rf-73-owned");
        if marker.exists() {
            ...
            // Preserve the legacy marker and all host settings. Never adopt an
        ...
        fs::write(library.join(".rf-73-owned"), MARKER)?;
    }
    Ok(())
}
...
fn ensure_no_newer_version(library: &Path) -> Result<(), Box<dyn Error>> {
    ...
    for entry in fs::read_dir(packages)? {
        ...
        {
            return Err("a newer RF-73 version is already in the audition library; refusing a misleading downgrade".into());
        }
    }
    Ok(())
}
...
mod tests {
    ...
    fn library_ownership_and_installed_bytes_are_checked() {
        ...
        fs::write(legacy.join(".rf-rhodes-owned"), LEGACY_MARKER).unwrap();
        fs::write(legacy.join("settings"), b"preserve").unwrap();
        own_library(&legacy).unwrap();
        own_library(&legacy).unwrap();
        assert_eq!(
            fs::read_to_string(legacy.join(".rf-73-owned")).unwrap(),
            MARKER
        );
        assert_eq!(
            fs::read_to_string(legacy.join(".rf-rhodes-owned")).unwrap(),
            LEGACY_MARKER
        );
        assert_eq!(fs::read(legacy.join("settings")).unwrap(), b"preserve");
        fs::write(legacy.join(".rf-73-owned"), b"conflicting owner").unwrap();
        assert!(own_library(&legacy).is_err());
        assert_eq!(
            fs::read(legacy.join(".rf-73-owned")).unwrap(),
            b"conflicting owner"
        );
