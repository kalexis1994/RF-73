...
fn soften_arm(p: &mut ElectromechanicalProfile) {
    let rest = rf_73_dsp::ElectromechanicalAssembly::new_at_rest(1e-6, *p)
        .expect("settled rest")
        .probe();
...
mod tests {
    ...
    fn soft_arm_matches_the_settled_rest_felt_force_to_first_order() {
        ...
        let settled = rf_73_dsp::ElectromechanicalAssembly::new_at_rest(1e-6, profile(position, 1))
            .unwrap()
            .probe();
        let soft = rf_73_dsp::ElectromechanicalAssembly::new_at_rest(1e-6, profile(position, 4))
            .unwrap()
            .probe();
