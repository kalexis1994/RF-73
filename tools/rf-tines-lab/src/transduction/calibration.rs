...
use rf_tines_analysis::{detect_timbre_onset, measure_timbre_profile, pitch_anchor};
use rf_tines_dsp::ElectromechanicalProfile;
...
mod tests {
    ...
    fn grid_preserves_upper_mode_losses_and_static_state() {
        ...
        let initial = rf_tines_dsp::ElectromechanicalAssembly::new_at_rest(1e-6, base)
            .unwrap()
            .probe();
        for t in TINE {
            for s in SUPPORT {
                ...
                assert_eq!(
                    rf_tines_dsp::ElectromechanicalAssembly::new_at_rest(1e-6, p)
                        .unwrap()
                        .probe(),
                    initial
                );
