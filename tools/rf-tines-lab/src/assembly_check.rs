...
use rf_tines_dsp::{AssemblyParameters, AssemblyVoice};
use serde::Serialize;
use std::{error::Error, io::Write, path::Path};

pub const HELP: &str = "Coupled assembly experiment:
  assembly-check --output REPORT.json
  assembly-refinement --output REPORT.json
...
Refinement compares midpoint with exponential free motion and 1/32/64 contact
substeps at all four output rates (48 cases), plus an isolated native timing probe.
";
...
struct Row {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    displacement_relative_rmse_vs_refined_64: Option<f64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    velocity_relative_rmse_vs_refined_64: Option<f64>,
}
...
fn take(
    ...
    let mut voice = match contact_substeps {
        Some(count) => AssemblyVoice::new_refined(f64::from(rate), count, p)?,
        None => AssemblyVoice::new(f64::from(rate), substeps, p)?,
    };
    ...
    let mut row = Row {
        ...
        displacement_relative_rmse_vs_refined_64: None,
        velocity_relative_rmse_vs_refined_64: None,
    };
    ...
    for note in [28, 40, 55, 69, 88, 100] {
        for rate in [44100, 192000] {
            for velocity in [0.01, 1.0] {
                let reference = take(note, rate, velocity, 256, None)?;
                for substeps in [4, 16, 64, 128] {
                    let mut candidate = take(note, rate, velocity, substeps, None)?;
...
pub fn run_refinement(args: &[String]) -> Result<(), Box<dyn Error>> {
    if args.len() != 3
        || args[1] != "--output"
        || Path::new(&args[2]).extension().is_none_or(|s| s != "json")
    {
    ...
    for note in [28, 40, 55, 69, 88, 100] {
        for rate in [44100, 48000, 96000, 192000] {
            for velocity in [0.01, 1.0] {
                let reference = take(note, rate, velocity, 256, None)?;
                let mut fine_contact = take(note, rate, velocity, 4, Some(64))?;
                for (substeps, contact) in [(4, None), (128, None), (4, Some(1)), (4, Some(32))] {
                    let mut candidate = take(note, rate, velocity, substeps, contact)?;
                    candidate.row.displacement_relative_rmse_vs_256 =
                        error(&candidate.q, &reference.q, candidate.row.masses_kg);
                    candidate.row.velocity_relative_rmse_vs_256 =
                        error(&candidate.v, &reference.v, candidate.row.masses_kg);
                    if contact.is_some() {
                        candidate.row.displacement_relative_rmse_vs_refined_64 = Some(error(
                            &candidate.q,
                            &fine_contact.q,
                            candidate.row.masses_kg,
                        ));
                        candidate.row.velocity_relative_rmse_vs_refined_64 = Some(error(
                            &candidate.v,
                            &fine_contact.v,
                            candidate.row.masses_kg,
                        ));
                ...
                fine_contact.row.displacement_relative_rmse_vs_256 =
                    error(&fine_contact.q, &reference.q, fine_contact.row.masses_kg);
                fine_contact.row.velocity_relative_rmse_vs_256 =
                    error(&fine_contact.v, &reference.v, fine_contact.row.masses_kg);
                fine_contact.row.displacement_relative_rmse_vs_refined_64 = Some(0.0);
                fine_contact.row.velocity_relative_rmse_vs_refined_64 = Some(0.0);
                rows.push(fine_contact.row);
                rows.push(reference.row);
            }
        }
    }
    let pass = rows.iter().all(|row| {
        row.maximum_balance_relative_residual < 1e-8
            ...
            && row.displacement_relative_rmse_vs_256.is_finite()
            && row.velocity_relative_rmse_vs_256.is_finite()
            && (row.contact_substeps != Some(32)
                || (row.displacement_relative_rmse_vs_256 < 0.001
                    && row.velocity_relative_rmse_vs_256 < 0.001
                    && row
                        .displacement_relative_rmse_vs_refined_64
                        .is_some_and(|e| e < 0.0001)
                    && row
                        .velocity_relative_rmse_vs_refined_64
                        .is_some_and(|e| e < 0.0001)))
    });
    let report = serde_json::json!({
        "schema_version": 1, "model": "provisional-common-support-assembly-v1",
        "experiment": "exponential-free-motion-and-contact-refinement-v1",
        "status": if pass { "pass" } else { "fail" },
        "calibrated": false, "plugin_integrated": false,
        "duration_seconds": 0.05, "damper_frame": "floor(output_frames / 2)",
        "reference_substeps": 256,
        "gates": {"energy_balance_relative": 1e-8, "positive_energy_step_relative": 1e-10, "refined_32_rmse_vs_256": 0.001, "refined_32_rmse_vs_refined_64": 0.0001},
        "scope": "Mechanical probes, finite reference, no pickup or filtering. Refined energy and separation observed at base ticks; force averaged over contact microsteps. SI parameters are unchanged. Native isolated timing is not plugin or host qualification.",
        "cases": rows,
    ...
    writeln!(output)?;
    if !pass {
        return Err("assembly refinement audit failed; see report".into());
    }
    println!(
        "Assembly refinement passed: 48 cases, 288 takes. Report: {}",
        args[2]
    );
    Ok(())
}
...
fn timing() -> Result<serde_json::Value, Box<dyn Error>> {
    ...
    for (substeps, contact) in [(4, None), (128, None), (4, Some(32))] {
        let mut runs = Vec::new();
        for _ in 0..5 {
            let p = AssemblyParameters::provisional(100)?;
            let mut voice = match contact {
                Some(count) => AssemblyVoice::new_refined(48000.0, count, p)?,
                None => AssemblyVoice::new(48000.0, substeps, p)?,
            };
            let start = Instant::now();
            for frame in 0..12000 {
                if frame % 2400 == 0 && !voice.strike(1.0) {
                    return Err("timing restrike rejected".into());
                }
                if frame % 2400 == 1200 {
                    voice.set_damped(true);
                }
            ...
            if !black_box(voice.probe()).mechanical_energy_j.is_finite() {
                return Err("non-finite timing result".into());
            }
            runs.push(elapsed);
        }
        runs.sort_by(f64::total_cmp);
        timings.push(
            serde_json::json!({"substeps": substeps, "contact_substeps": contact,
            "minimum_seconds": runs[0], "median_seconds": runs[2], "maximum_seconds": runs[4]}),
        );
    }
    Ok(
        serde_json::json!({"profile": if cfg!(debug_assertions) { "debug" } else { "release" },
        "note": 100, "sample_rate_hz": 48000, "duration_seconds": 0.25, "strikes": 5,
        "runs_per_integrator": 5, "preparation_timed": false, "energy_ledger_enabled": true,
        "scope": "One native voice including repeated attacks and binary damping, no pickup, no host, no real-time deadline claim.",
        "integrators": timings}),
    )
}
