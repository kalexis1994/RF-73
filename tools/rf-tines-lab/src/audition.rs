...
// Host identity for the renamed instrument. Packages, sessions and custom
// programs stored under an earlier identity do not resolve to this one.
const ID: &str = "org.rackforge.rftines";
const INSTANCE: &str = "desktop.org.rackforge.rftines";
const MARKER: &str = "RF-Tines audition library v1\n";
// Ownership markers written under the project's earlier names, newest first.
// They only authorize adopting a library this project already owned; the
// instrument identity itself is not carried over.
const LEGACY_MARKERS: [(&str, &str); 2] = [
    (".rf-73-owned", "RF-73 audition library v1\n"),
    (".rf-rhodes-owned", "RF-Rhodes audition library v1\n"),
];
...
pub fn run(args: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "Building RF-Tines {} for audition...",
        env!("CARGO_PKG_VERSION")
    );
    ...
    let archive = run_dir.join(format!("RF-Tines-{}.rfplugin", env!("CARGO_PKG_VERSION")));
    build_to(&archive)?;
    ...
    verify_install(
        &installed,
        &root.join("target/wasm32-unknown-unknown/release/rf_tines_plugin.wasm"),
    )?;
    ...
    if let Some(desktop) = desktop {
        ...
        println!(
            "RackForge started: PID {}. RF-Tines {} installed and selected.",
            child.id(),
            env!("CARGO_PKG_VERSION")
        );
        ...
        write_json(&run_dir.join("audition.json"), &receipt)?;
        println!(
            "Prepared RF-Tines {} without launching Desktop.",
            env!("CARGO_PKG_VERSION")
        );
    }
...
fn find_desktop(host: &Path) -> Result<PathBuf, Box<dyn Error>> {
    if let Some(path) = std::env::var_os("RF_TINES_DESKTOP")
        .or_else(|| std::env::var_os("RF_73_DESKTOP"))
        .or_else(|| std::env::var_os("RF_RHODES_DESKTOP"))
    {
        let path = PathBuf::from(path);
        if !path.is_file() {
            return Err("RF_TINES_DESKTOP must name an existing RackForge executable".into());
        }
    ...
    .ok_or("build RackForge Desktop first, or set RF_TINES_DESKTOP to its executable")?;
...
fn own_library(library: &Path) -> Result<(), Box<dyn Error>> {
    if library.exists() {
        ...
        let marker = library.join(".rf-tines-owned");
        if marker.exists() {
            if fs::read_to_string(&marker)?.as_str() != MARKER {
                return Err("refusing to modify an unmarked audition library".into());
            }
        } else {
            let adopted = LEGACY_MARKERS.iter().any(|(name, legacy)| {
                fs::read_to_string(library.join(name)).ok().as_deref() == Some(*legacy)
            });
            if !adopted {
                return Err("refusing to modify an unmarked audition library".into());
            }
            // Preserve every legacy marker and all host settings. Never adopt an
            // arbitrary directory or overwrite a conflicting new marker.
            OpenOptions::new()
                .write(true)
                .create_new(true)
                .open(marker)?
        ...
        fs::write(library.join(".rf-tines-owned"), MARKER)?;
    }
    Ok(())
}
...
fn ensure_no_newer_version(library: &Path) -> Result<(), Box<dyn Error>> {
    ...
    for entry in fs::read_dir(packages)? {
        ...
        {
            return Err("a newer RF-Tines version is already in the audition library; refusing a misleading downgrade".into());
        }
    }
    Ok(())
}
...
fn checkpoint(previous: Option<Value>) -> Result<Value, Box<dyn Error>> {
    ...
    // their selected instrument sound and saved instrument settings.
...
mod tests {
    use super::*;
    #[test]
    fn checkpoint_selects_rftines_and_preserves_user_controls() {
    ...
    fn library_ownership_and_installed_bytes_are_checked() {
        ...
        own_library(&owned).unwrap();
        // Every earlier ownership marker is adopted, preserved and not replaced.
        for (index, (name, marker)) in LEGACY_MARKERS.iter().enumerate() {
            let legacy = path.join(format!("legacy-{index}"));
            fs::create_dir(&legacy).unwrap();
            fs::write(legacy.join(name), marker).unwrap();
            fs::write(legacy.join("settings"), b"preserve").unwrap();
            own_library(&legacy).unwrap();
            own_library(&legacy).unwrap();
            assert_eq!(
                fs::read_to_string(legacy.join(".rf-tines-owned")).unwrap(),
                MARKER
            );
            assert_eq!(fs::read_to_string(legacy.join(name)).unwrap(), *marker);
            assert_eq!(fs::read(legacy.join("settings")).unwrap(), b"preserve");
        }
        let legacy = path.join("legacy-0");
        fs::write(legacy.join(".rf-tines-owned"), b"conflicting owner").unwrap();
        assert!(own_library(&legacy).is_err());
        assert_eq!(
            fs::read(legacy.join(".rf-tines-owned")).unwrap(),
            b"conflicting owner"
        );
