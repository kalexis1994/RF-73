const FORMAT = "rackforge.rf73-program";
...
export function validateProgram(program) {
  ...
  if (![4, 5].includes(program.plugin_state_version)) {
    throw new Error("Unsupported RF-73 program version.");
  }
  ...
  if (!exactKeys(program.payload, [...CORE_FIELDS, ...ELECTRONICS_FIELDS], requiredPayload)) {
    throw new Error("Unknown or missing RF-73 parameters.");
  }
  ...
  if (!valid) throw new Error("One or more RF-73 parameters are outside their valid range.");
  return structuredClone(program);
}

export async function createRf73File(program) {
...
export async function parseRf73File(text) {
  if (typeof text !== "string" || new TextEncoder().encode(text).byteLength > MAX_FILE_BYTES) {
    throw new Error("The .rf73 file is larger than 32 KiB.");
  }
  let file;
  try {
    file = JSON.parse(text);
  } catch {
    throw new Error("The .rf73 file is not valid JSON.");
  }
  if (!exactKeys(file, ["format", "schema_version", "program", "integrity"])) {
    throw new Error("Unknown or missing .rf73 file fields.");
  }
  if (file.format !== FORMAT || file.schema_version !== 1) throw new Error("Unsupported .rf73 file format.");
  if (!exactKeys(file.integrity, ["algorithm", "digest"]) || file.integrity.algorithm !== "SHA-256") {
    throw new Error("Unsupported .rf73 integrity record.");
  }
  if (!/^[0-9a-f]{64}$/.test(file.integrity.digest)) throw new Error("The .rf73 checksum is invalid.");
  const program = validateProgram(file.program);
  if ((await digest(program)) !== file.integrity.digest) throw new Error("The .rf73 file is damaged or was modified.");
  return program;
}

export const RF73_MAX_FILE_BYTES = MAX_FILE_BYTES;
